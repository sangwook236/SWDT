#include "../open_robotics_ars_lib/ARSByEKFAndRotationMatrix.h"
#include "../open_robotics_ars_lib/ARSByEKFAndQuaternion.h"
#include "../open_robotics_ars_lib/ARSByComplementaryFilterAndRotationMatrix.h"
#include "../open_robotics_ars_lib/ARSByComplementaryFilterAndQuaternion.h"
...
public:
	serial_port_handler_for_mpu6050(boost::asio::io_service &io_service, boost::scoped_ptr<ARS> &ars)
	: base_type(io_service), ars_(ars),
	  imu_data_list_(20)
	{
		// FIXME [delete] >> for test.
		outStream_.open("./data/ahrs/mpu6050_open_robotics_ars.dat");
...
private:
	...
	void process_packet(const std::string &packet)
	{
		...
		if (values.size() == 6)
		{
			...
			imu_data.gyrox = values[3] / GYROSCOPE_SENSITIVITY;
			imu_data.gyroy = values[4] / GYROSCOPE_SENSITIVITY;
			imu_data.gyroz = values[5] / GYROSCOPE_SENSITIVITY;
			imu_data.magx = 0.0;
			imu_data.magy = 0.0;
			imu_data.magz = 0.0;
#else
			...
			imu_data.gyrox = values[5] / GYROSCOPE_SENSITIVITY;
			imu_data.gyroy = values[4] / GYROSCOPE_SENSITIVITY;
			imu_data.gyroz = values[3] / GYROSCOPE_SENSITIVITY;
			imu_data.magx = 0.0;
			imu_data.magy = 0.0;
			imu_data.magz = 0.0;
#endif
			...
			if (ars_)
			{
				...
				const dMatrix &R = ars_->filter(&imu_data_final.gyrox, &imu_data_final.accelx, &imu_data_final.magx, dt);
				//std::cout << R << std::endl;
...
private:
	const boost::scoped_ptr<ARS> &ars_;
...
void filter_by_ars_using_mpu6050()
{
	try
	{
		//boost::scoped_ptr<ARS> ars(new ARSByEKFAndRotationMatrix());
		//boost::scoped_ptr<ARS> ars(new ARSByEKFAndQuaternion());
		//boost::scoped_ptr<ARS> ars(new ARSByComplementaryFilterAndRotationMatrix());
		boost::scoped_ptr<ARS> ars(new ARSByComplementaryFilterAndQuaternion());
		if (ars)
			ars->initialize();
		else
		{
			std::cerr << "ARS not created" << std::endl;
			return;
		}
		...
		const unsigned int baud_rate = 38400;
		{
			...
			serial_port_handler_for_mpu6050 handler(ioService, ars);
...
namespace my_open_robotics_ars {

}  // namespace my_open_robotics_ars

int open_robotics_ars_main(int argc, char *argv[])
{
	local::filter_by_ars_using_mpu6050();
