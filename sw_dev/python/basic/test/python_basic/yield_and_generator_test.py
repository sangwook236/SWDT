...
def shuffle_generator():
	...
	# NOTE [info] >> Elements had better be shuffled in a generator.

	def gen_n(n):
		for v in range(n):
			print('gen_n() is called: v = {}.'.format(v))
			yield v
	...
	for idx, val in enumerate(gen_n(N)):
		print(idx, val)

	#--------------------
	#for val in random.sample(gen_n(N), N):  # TypeError: Population must be a sequence or set.
	for val in random.sample(list(gen_n(N)), N):
		print(val)

	#--------------------
	# All elements are generated by a generator before sorted() is applied. (Not so good)
	#print(type(sorted(gen_n(N), key=lambda k: random.random())))
	#print(len(sorted(gen_n(N), key=lambda k: random.random())))
	#print(sorted(gen_n(N), key=lambda k: random.random()))

	#print(list(val for val in sorted(gen_n(N), key=lambda k: random.random())))
	for val in sorted(gen_n(N), key=lambda k: random.random()):
		print(val)
