...
if False:
	...
	fc1_drop = tf.nn.dropout(fc1_hidden, keep_prob_ph)
	...
	conv1_hidden = slim.conv2d(x_tensor, 32, [5, 5], activation_fn=tf.nn.relu)
	conv1_pool = slim.max_pool2d(conv1_hidden, [2, 2])
	conv2_hidden = slim.conv2d(conv1_pool, 64, [5, 5], activation_fn=tf.nn.relu)
	conv2_pool = slim.max_pool2d(conv2_hidden, [2, 2])
	conv2_flat = slim.flatten(conv2_pool)
	fc1 = slim.fully_connected(conv2_flat, 1024, activation_fn=tf.nn.relu)
	fc1_drop = slim.dropout(fc1, keep_prob_ph)
	y_pred = slim.fully_connected(fc1_drop, 10, activation_fn=tf.nn.softmax)
...
for i in range(1001):
	...
	sess.run(train_step, feed_dict={x_image_ph: batch[0], y_true_ph: batch[1], keep_prob_ph: 0.5})
	if i % 100 == 0 and i != 0:
		trainAccuracy = sess.run(accuracy, feed_dict={x_image_ph: batch[0], y_true_ph: batch[1], keep_prob_ph: 1.0})
		print('Step %d, training accuracy %g' % (i, trainAccuracy))

testAccuracy = sess.run(accuracy, feed_dict={x_image_ph: mnist.test.images, y_true_ph: mnist.test.labels, keep_prob_ph: 1.0})
...
def getActivations(layer, stimuli):
	units = sess.run(layer, feed_dict={x_image_ph: np.reshape(stimuli, [1, 784], order='F'), keep_prob_ph: 1.0})  # units -> numpy.array.
	plotNNFilter(units)
