...
from tensorflow.keras.layers import Input, Conv2D, Conv2DTranspose, Dense, Reshape, Flatten, Embedding, Dropout
...
def build_generator(latent_size, num_classes):
	# We will map a pair of (z, L), where z is a latent vector and L is a label drawn from P_c, to image space (..., 28, 28, 1).
	cnn = Sequential()
...
def build_discriminator():
	# Build a relatively standard conv net, with LeakyReLUs as suggested in the reference paper.
	cnn = Sequential()
...
# REF [site] >>
#	https://keras.io/examples/mnist_acgan/
#	https://github.com/keras-team/keras/blob/master/examples/mnist_acgan.py
# REF [paper] >> "Conditional Image Synthesis With Auxiliary Classifier GANs", arXiv 2016.
def auxiliary_classifer_gan_mnist_example():
	np.random.seed(1337)
	num_classes = 10
...
def main():
	auxiliary_classifer_gan_mnist_example()
