...
import os, itertools, re, codecs, datetime
import numpy as np
...
from tensorflow.keras.layers import Conv2D, MaxPooling2D, GRU
...
def paint_text(text, w, h, rotate=False, ud=False, multi_fonts=False):
	...
	with cairo.Context(surface) as context:
		...
		# Teach the RNN translational invariance by fitting text box randomly on canvas, with some room to rotate.
...
# Only a-z and space..probably not to difficult to expand to uppercase and symbols.
def is_valid_str(in_str):
    search = re.compile(regex, re.UNICODE).search
    return bool(search(in_str))

# Uses generator functions to supply train/test with data.
# Image renderings and text are created on the fly each time with random perturbations.
class TextImageGenerator(keras.callbacks.Callback):
	...
	# Each time an image is requested from train/val/test, a new random painting of the text is performed.
	def get_batch(self, index, size, train):
		# Width and height are backwards from typical Keras convention because width is the time dimension when it gets fed into the RNN.
		...
		source_str = []
		for i in range(size):
			# Mix in some blank inputs. This seems to be important for achieving translational invariance.
