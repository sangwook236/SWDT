...
class EncoderLayer(torch.nn.Module):
	...
	def forward(self, x, mask):
		...
		x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, mask))  # mask: [batch_size, 1, 1, seq_len]
...
class DecoderLayer(torch.nn.Module):
	...
	def forward(self, x, memory, src_mask, tgt_mask):
		...
		m = memory
		x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, tgt_mask))  # tgt_mask: [batch_size, 1, seq_len - 1, seq_len - 1]
		x = self.sublayer[1](x, lambda x: self.src_attn(x, m, m, src_mask))  # src_mask: [batch_size, 1, 1, seq_len]
...
def attention(query, key, value, mask=None, dropout=None):  # query: [batch_size, num_heads, query_seq_len, d_model / num_heads], key: [batch_size, num_heads, key_seq_len, d_model / num_heads], value: [batch_size, num_heads, value_seq_len, d_model / num_heads]
