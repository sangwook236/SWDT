...
const std::string data1_file = ".\\ann_data\\test1-data.pts";
const std::string query1_file = ".\\ann_data\\test1-query.pts";
const std::string data2_file = ".\\ann_data\\test2-data.pts";
const std::string query2_file = ".\\ann_data\\test2-query.pts";
...
int main(int argc, char **argv)
{
	...
	case 0:
		...
		break;
	case 1:
		dataStream.open(data1_file.c_str(), std::ios::in);
		queryStream.open(query1_file.c_str(), std::ios::in);
		if (!dataStream || !queryStream) return 1;
		dataIn = &dataStream;
		queryIn = &queryStream;
		k = 1;
		dim = 2;
		...
		break;
	case 2:
		dataStream.open(data2_file.c_str(), std::ios::in);
		queryStream.open(query2_file.c_str(), std::ios::in);
		if (!dataStream || !queryStream) return 1;
		dataIn = &dataStream;
		queryIn = &queryStream;
		k = 3;
...
void getArgs(int argc, char **argv)
{
	...
	while (i < argc)							// read arguments
	{
		...
		{
			dataStream.open(argv[++i], std::ios::in);	// open data file
			if (!dataStream)
			{
		...
		{
			queryStream.open(argv[++i], std::ios::in);	// open query file
			if (!queryStream)
			{
