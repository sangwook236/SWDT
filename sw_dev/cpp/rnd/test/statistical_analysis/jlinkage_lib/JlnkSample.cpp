...
namespace JlnkSample {
	...
	float (*mPtPairDistance)(const std::vector<float> &nVecI, const std::vector<float> &nVecJ);
	...
	{ 
		...
		case MT_VP:
			...
			mPtPairDistance = PtPairDistance_VP;
			break;
		default: printf("Invalid model type"); return 0; break;
		}

		RandomSampler mRandomSampler(mGetFunction, mDistanceFunction,mPtPairDistance, (int)(*(*mDataPoints)[0]).size()-1, mMSS, (int)mDataPoints->size(),true);
		...
		std::vector<std::vector<float> *> *mModels = 
			...
			mRandomSampler.GetNSample(mNSample, 0, NULL, NULL);
			//--E [] 2015/02/15 : Sang-Wook Lee
		//--S [] 2015/02/15 : Sang-Wook Lee
		//CloseWaitbar();
		//--S [] 2015/02/15 : Sang-Wook Lee
