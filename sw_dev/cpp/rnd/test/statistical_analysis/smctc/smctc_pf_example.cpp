...
void pf_example()
{
	...
	// Load observations
	const long lIterates = local::load_data("./data/statistical_analysis/data.csv", &pffuncs::y);
	...
	for (int n = 1; n < lIterates; ++n)
	{
		...
		const double xm = Sampler.Integrate(local::integrand_mean_x, NULL);
		const double xv = Sampler.Integrate(local::integrand_var_x, (void *)&xm);
		const double ym = Sampler.Integrate(local::integrand_mean_y, NULL);
		const double yv = Sampler.Integrate(local::integrand_var_y, (void *)&ym);
