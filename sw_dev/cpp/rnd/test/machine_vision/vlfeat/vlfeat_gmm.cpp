...
void gmm()
{
	...
	//char *dataFileResults = "./data/machine_vision/vlfeat/gmm-results.mat";
	//char *dataFileData = "./data/machine_vision/vlfeat/gmm-data.mat";
	...
	vl_free(data);
	numData = 2000;
	data = (TYPE *)vl_malloc(numData * dimension * sizeof(TYPE));
	for (dataIdx = 0; dataIdx < numData; ++dataIdx)
	{
		for (d = 0; d < dimension; ++d)
		{
	...
	vl_uint32 *assign = (vl_uint32 *)vl_malloc(numData * numClusters * sizeof(vl_uint32));
	for (dataIdx = 0; dataIdx < numData; ++dataIdx)
	{
		for (cIdx = 0; cIdx < numClusters; ++cIdx)
		{
	...
	{
		vl_free(enc);
		enc = (TYPE *)vl_malloc(sizeof(TYPE) * dimension * numClusters);
		vl_vlad_encode(
			enc, VL_F_TYPE,
			vl_gmm_get_means(gmm), dimension, numClusters,
			data, numData,
