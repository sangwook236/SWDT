...
void homography()
{
	...
	local::extract_features(targetImage, targetStorage, targetKeypoints, targetDescriptors, useExtendedDescriptor);
	IplImage *inputImage = cvLoadImage(inputImageFileName.c_str());
	CvSeq *inputKeypoints = NULL;
	CvSeq *inputDescriptors = NULL;
	local::extract_features(inputImage, inputStorage, inputKeypoints, inputDescriptors, useExtendedDescriptor);
	...
	local::match_using_k_nearest_neighbor(targetKeypoints, targetDescriptors, inputKeypoints, inputDescriptors, descriptorLength, distanceThreshold, matchedPairs);
	...
	if (!local::calc_homography(targetKeypoints, inputKeypoints, matchedPairs, homography))
	{
