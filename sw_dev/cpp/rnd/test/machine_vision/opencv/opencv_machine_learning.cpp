...
void train_by_svm()
{
	...
	//
	CvMat *trainInput = cvCreateMat(local::trainingSampleNum, local::trainingFeatureNum, CV_32FC1);
	CvMat *trainOutput = cvCreateMat(local::trainingSampleNum, 1, CV_32FC1);
	for (size_t i = 0; i < local::trainingSampleNum; ++i)
	{
		for (size_t j = 0; j < local::trainingFeatureNum; ++j)
		{
	...
	//
	CvMat *predictInput = cvCreateMat(1, local::trainingFeatureNum, CV_32FC1);
	std::vector<float> predictOutput(local::trainingSampleNum);
	std::vector<int> predictInputIdx(local::trainingSampleNum);
	for (size_t i = 0; i < local::trainingSampleNum; ++i)
	{
		predictInputIdx[i] = rand() % local::trainingSampleNum;
...
void train_by_ann()
{
	const size_t inputLayerNeuronNum = local::trainingFeatureNum;
	...
	//
	CvMat *trainInput = cvCreateMat(local::trainingSampleNum, inputLayerNeuronNum, CV_32FC1);
	CvMat *trainOutput = cvCreateMat(local::trainingSampleNum, ouputLayerNeuronNum, CV_32FC1);
	for (size_t i = 0; i < local::trainingSampleNum; ++i)
	{
	...
	//
	CvMat *predictOutput = cvCreateMat(local::trainingSampleNum, ouputLayerNeuronNum, CV_32FC1);
