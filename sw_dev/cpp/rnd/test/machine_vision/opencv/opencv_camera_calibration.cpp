...
void StereoCalib(const std::vector<std::string> &imagelist, const cv::Size &boardSize, const float squareSize, const int maxScale, const bool useCalibrated, const bool showRectified, const bool displayCorners)
{
	...
	cv::FileStorage fs("./machine_vision_data/opencv/camera_calibration/intrinsics.yml", CV_STORAGE_WRITE);
	if (fs.isOpened())
	{
	...
	fs.open("./machine_vision_data/opencv/camera_calibration/extrinsics.yml", CV_STORAGE_WRITE);
	if (fs.isOpened())
	{
