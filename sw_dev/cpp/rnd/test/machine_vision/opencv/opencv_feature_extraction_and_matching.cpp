...
void feature_extraction_and_matching()
{
	...
	if (evaluationMode)
	{
		local::warpPerspectiveRand(img1, img2, H12, rng);
		if (img2.empty())
		{
	...
	{
	case 0:
		local::simpleMatching(descriptorMatcher, descriptors1, descriptors2, filteredMatches);
		break;
	case 1:
		local::crossCheckMatching(descriptorMatcher, descriptors1, descriptors2, filteredMatches, 1);
		break;
	}
