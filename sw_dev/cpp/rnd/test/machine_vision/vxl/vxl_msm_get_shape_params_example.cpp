...
void tool_params::read_from_file(const vcl_string &path)
{
	...
	output_path = props.get_optional_property("output_path", "shape_params.txt");
...
// [ref] ${VXL_HOME}/contrib/mul/msm/tools/msm_get_shape_params.cxx
int msm_get_shape_params_example(int argc, char *argv[])
{
	...
	if (!ofs)
	{
		vcl_cerr << "Failed to open " << params.output_path << " for output." << vcl_endl;
		return 1;
	}
	...
	for (unsigned i = 0; i < shapes.size(); ++i)
	{
		...
		if (write_bestfitpts)
		{
			...
			{
				vcl_cout << "Directory " << out_pts_dir << " does not exist. Creating it." << vcl_endl;
