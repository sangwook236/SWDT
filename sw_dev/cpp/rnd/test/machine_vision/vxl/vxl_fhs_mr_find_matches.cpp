...
void print_usage()
{
	vcl_cout << "find_matches -i1 image1.jpg -i2 image2.jpg -Llo 0 Lhi 2\n"
...
// [ref] ${VXL_HOME}/contrib/mul/fhs/tools/mr_find_matches.cxx
int fhs_mr_find_matches_example(int argc, char *argv[])
{
	...
	local::get_strongest_corners(image_pyr1(level_hi()), pts,nc());
	...
	local::extract_normalised_patches(image_pyr1(level_hi()), pts,w(), patch, patch_ref);
...
unsigned i0 = 0;  // Record index of first point at level above
	...
	for (int L = level_hi() - 1; L >= level_lo(); --L)
	{
		...
		// Search the coarsest image for corners
		local::get_strongest_corners(image_pyr1(L), L_pts, max_pts);

		// Extract patches for each point, pushing back onto patch,patch_ref
		local::extract_normalised_patches(image_pyr1(L), L_pts,w(), patch,patch_ref);
		...
		for (unsigned i = 0; i < L_pts.size(); ++i)
		{
			...
			vcl_pair<int, int> pair_i(pts.size() - 1, local::closest_pt_index(pts, 0, i1, L_pts[i]));
	...
	// Draw tree into image for display purposes
	local::draw_tree(image1.image(), pts, pairs);
	...
	for (unsigned i = 0; i < pts.size(); ++i)
	{
		...
		if (im_level[i] != (unsigned int)level_lo())
		{
			...
			local::vimt_resample(fr, im_level[i], feature_response[i], level_lo());
		}
