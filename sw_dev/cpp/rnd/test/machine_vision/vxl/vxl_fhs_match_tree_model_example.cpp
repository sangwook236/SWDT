...
void print_usage()
{
	vcl_cout << "fhs_match_tree_model -p param_file\n"
		...
		<< "equivalent points on the second image."
		<< vcl_endl;
...
bool parse_param_file(const vcl_string &param_path, fhs_model_params &params)
{
	...
	{
		vcl_cerr << "No image1_path: specified." << vcl_endl;
		return false;
	}
	...
	{
		vcl_cerr << "No image2_path: specified." << vcl_endl;
		return false;
	}
	...
	{
		vcl_cerr << "No output_image1_path: specified." << vcl_endl;
		return false;
	}
	...
	{
		vcl_cerr << "No output_image2_path: specified." << vcl_endl;
		return false;
	}
	...
	{
		vcl_cerr << "No points_path: specified." << vcl_endl;
		return false;
	}
	...
	{
		vcl_cerr << "No arcs2_path: specified." << vcl_endl;
		return false;
	}
	...
	{
		vcl_cerr << "No L_lo: specified." << vcl_endl;
		return false;
	}
	...
	{
		vcl_cerr << "No half_width: specified." << vcl_endl;
		return false;
	}
	...
	{
		vcl_cerr << "No L_hi: specified." << vcl_endl;
		return false;
	}
...
bool fhs_load_points(const vcl_string &path, vcl_vector<vgl_point_2d<double> > &pts)
{
	...
	if (!ifs)
	{
		vcl_cerr << "Failed to load in points from " << path << vcl_endl;
		return false;
	}
...
bool fhs_load_arcs(const vcl_string &path, vcl_vector<vcl_pair<int, int> > &pairs)
{
	...
	if (!ifs)
	{
		vcl_cerr << "Failed to load in arc links from " << path << vcl_endl;
		return false;
	}
...
void extract_normalised_patches(
	const vimt_image &image, const vcl_vector<vgl_point_2d<double> > &pts, int half_width,
	vcl_vector<vil_image_view<float> > &patch, vcl_vector<vgl_point_2d<double> > &ref_pts
)
{
	const vimt_image_2d_of<vxl_byte> &byte_im = static_cast<const vimt_image_2d_of<vxl_byte> &>(image);
	...
	for (unsigned i = 0; i < n ;++i)
	{
		...
		vil_convert_cast(vil_crop(byte_im.image(), ilo, 1 + ihi - ilo, jlo, 1 + jhi - jlo), patch1);
...
void draw_tree(vil_image_view<vxl_byte> &image, const vcl_vector<vgl_point_2d<double> > &pts, const vcl_vector<vcl_pair<int, int> > &pairs)
{
	...
	for (unsigned i = 0; i < pairs.size(); ++i)
		mbl_draw_line(image, pts[pairs[i].first], pts[pairs[i].second], vxl_byte(255));

	// Write position of selected points into the original image for display purposes.
...
// [ref] ${VXL_HOME}/contrib/mul/fhs/tools/fhs_match_tree_model.cxx
int fhs_match_tree_model_example(int argc, char *argv[])
{
	...
	if (!local::fhs_load_points(params.points_path, ref_pts)) return 3;
	...
	if (!local::fhs_load_arcs(params.arcs_path, pairs)) return 4;
	...
	for (unsigned i = 0; i < pairs.size(); ++i)
	{
		...
		{
			vcl_cerr << "Invalid point index " << pairs[i].first << vcl_endl;
			return 5;
		}
		if (pairs[i].second < 0 || (unsigned int)(pairs[i].second) >= ref_pts.size())
		{
			vcl_cerr << "Invalid point index " << pairs[i].second << vcl_endl;
			return 5;
		}
	}
	...
	local::extract_normalised_patches(image_pyr1(params.L_hi), ref_pts, params.half_width, patch, patch_ref);
	...
	{
		const vimt_image_2d_of<vxl_byte> &byte_im = static_cast<const vimt_image_2d_of<vxl_byte> &>(image_pyr2(im_level[i]));
		...
		vimt_image_2d_of<vxl_byte> cropped_im = vimt_crop(byte_im, ilo, 1 + ihi - ilo, jlo, 1 + jhi - jlo);

		// Apply to whole image in first instance
		// Ideally would crop a region around expected position
		vimt_normalised_correlation_2d(cropped_im, feature_response[i], patch[i], patch_ref[i], float());
		if (int(im_level[i]) != params.L_lo)
		{
			...
			local::vimt_resample(fr, im_level[i], feature_response[i], params.L_lo);
		}
	...
	// Draw tree into image for display purposes
	local::draw_tree(image1.image(), ref_pts,pairs);
