...
void tool_params::read_from_file(const vcl_string &path)
{
	...
	shape_model_path = props.get_optional_property("shape_model_path", "shape_aam.bfs");
...
void draw_mode(msm_shape_mode_view &mode_view, const msm_curves& curves, tool_params &params)
{
	...
	mbl_eps_writer writer(ss.str().c_str(), win_box.width(), win_box.height());
	...
	for (unsigned i = 0; i < n_shapes; ++i)
	{
		...
		if (params.point_radius>0)
			msm_draw_points_to_eps(writer,mode_view.points()[i], params.point_radius);
...
void draw_mode_frames(msm_shape_mode_view &mode_view, const msm_curves &curves, tool_params &params)
{
	mode_view.compute_shapes();
	unsigned m = mode_view.mode();
	vgl_box_2d<int> win_box = mode_view.display_window();
	...
	for (unsigned i = 0; i < 2 * n_shapes - 2; ++i)
	{
		...
		writer.set_colour(params.point_colour);
		if (params.point_radius > 0)
			msm_draw_points_to_eps(writer, mode_view.points()[f], params.point_radius);
		writer.set_colour(params.line_colour);
		writer.set_line_width(params.line_width);
	...
	vcl_cout << "Saved " << 2 * n_shapes - 2 << " frames to " << params.base_name << "_s" << m << "_XX.eps" << vcl_endl;
}
...
msm_shape_model create_subspace_model(const msm_shape_model &shape_model, const vcl_string &mode_path, const vcl_string &var_path)
{
	...
	{
		vcl_cerr << "Number of variances = " << new_var.size() << vcl_endl << "Numbers differ." << vcl_endl;
		vcl_abort();
	}
	...
	new_model.set(shape_model.mean_points(), newP, new_var, shape_model.default_pose(), shape_model.aligner(), shape_model.param_limiter());
...
// [ref] ${VXL_HOME}/contrib/mul/msm/tools/msm_draw_shape_modes.cxx
int msm_draw_shape_modes_example(int argc, char *argv[])
{
	...
	{
		vcl_cerr << "Failed to load shape model from " << params.shape_model_path << vcl_endl;
		return 2;
	}
	...
	if (params.subspace_mode_path != "")
		shape_model = local::create_subspace_model(shape_model, params.subspace_mode_path, params.subspace_var_path);
