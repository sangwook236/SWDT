...
// [ref] ${VXL_HOME}/contrib/mul/msm/tools/msm_apply_tps_warp.cxx
int msm_apply_tps_warp_example(int argc, char *argv[])
{
	...
	if (src_con_pts_path() == "" || dest_con_pts_path() == "" || src_pts_path() == "")
	{
		local::print_usage();
		return 0;
	}
	...
	{
		vcl_cerr << "Failed to load points from " << src_con_pts_path() << vcl_endl;
		return 1;
	}
	...
	{
		vcl_cerr << "Failed to load points from " << dest_con_pts_path() << vcl_endl;
		return 1;
	}
	...
	{
		vcl_cerr << "Failed to load points from " << src_pts_path() << vcl_endl;
		return 1;
	}
	...
	{
		vcl_cerr << "Failed to write points to " << dest_pts_path() << vcl_endl;
		return 2;
	}
