...
void tool_params::read_from_file(const vcl_string &path)
{
	...
	{
		vcl_string limiter_str = props.get_optional_property("param_limiter", "msm_ellipsoid_limiter { accept_prop: 0.98 }");
...
// [ref] ${VXL_HOME}/contrib/mul/msm/tools/msm_build_shape_model.cxx
int msm_build_shape_model_example(int argc, char *argv[])
{
	...
	builder.build_from_files(params.points_dir, params.points_names, shape_model);

	vcl_cout << "Built model: " << shape_model << vcl_endl;

	if (vsl_quick_file_save(shape_model, params.shape_model_path))
	{
		vcl_cout << "Saved shape model to " << params.shape_model_path << vcl_endl;
	}
	else
	{
