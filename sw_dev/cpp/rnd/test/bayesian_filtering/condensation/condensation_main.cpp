#include "../condensation_lib/data_types.h"
#include "../condensation_lib/model_parameters.h"
#include "../condensation_lib/condensation.h"
#include <iostream>
...
void update_after_iterating(int iteration)
{
	...
	Sample *temp = data.new_positions;
...
{
	for (int i = 0; i < global.niterations; ++i)
	{
...
int initialise(int argc, char *argv[])
{
	...
	data.new_positions = new Sample [global.nsamples];
	data.old_positions = new Sample [global.nsamples];

	data.sample_weights = new double [global.nsamples];
	data.cumul_prob_array = new double [global.nsamples];

	if (!data.new_positions || !data.old_positions ||
		!data.sample_weights || !data.cumul_prob_array)
	{
		std::cerr << "Failed to allocate memory for sample arrays" << std::endl;
		return 0;
	}
...
void shut_down()
{
	delete [] data.new_positions;
	delete [] data.old_positions;
	delete [] data.sample_weights;
	delete [] data.cumul_prob_array;
}

}  // namespace local
}  // unnamed namespace

namespace my_condensation {

}  // namespace my_condensation

int condensation_main(int argc, char *argv[])
{
	if (!local::initialise(argc, argv))
		return 1;
