...
#include <string.h>
#include <assert.h>
#include <math.h>  //-- [] 2012/07/12: Sang-Wook Lee
#include "mrf.h"
...
void MRF::checkArray(CostVal *V)
{
    ...
    for (i=0; i< m_nLabels; i++)
		...
		//--S [] 2012/07/12: Sang-Wook Lee
        //if (V[i*m_nLabels + j] != V[j*m_nLabels + i])
		if (fabs(double(V[i*m_nLabels + j] - V[j*m_nLabels + i])) > 1e-20)
		//--E [] 2012/07/12
        { 
            fprintf(stderr, "%d: %d, %d\n", m_nLabels, V[i*m_nLabels + j], V[j*m_nLabels + i]); 
