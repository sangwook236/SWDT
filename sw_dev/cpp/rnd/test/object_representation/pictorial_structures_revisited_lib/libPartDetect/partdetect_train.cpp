...
namespace part_detect { 
  ...
  */
  static void make_bbox(const PartWindowParam &window_params, int pidx, 
...
//               make_bbox(part_app.m_window_param, pidx, 
//                         vi(0), vi(1), 
//                         scale_from_index(exp_param, scaleidx), 
//                         rot_from_index(exp_param, rotidx)*M_PI/180.0,
//                         bbox);
...
//           qsScale = "0" + qsScale;
        ...
        QString qsFilename = qsDebugDir + "/bootstrap_fp_imgidx_" + padZeros(QString::number(imgidx), 4) + 
          "_pidx" + padZeros(QString::number(pidx), 2) + 
  ...
  */
  ...
  static kma::ImageContent* crop_part_region(kma::ImageContent *image, const PartBBox &bbox, int desc_size) {

    /** MA: Should not we add 2*desc_size here? Also, what happens if part center != center of part bounding box ? */
      ...
      */
      ...
      for (uint imgidx = 0; imgidx < poslist.size(); ++imgidx) 
        for (uint ridx = 0; ridx < poslist[imgidx].size(); ++ridx)
          if (annorect_has_part(poslist[imgidx][ridx], part_def)) 
            ++num_avail_pos;
      ...
      if (abc_param.do_jitter()) {
        num_jitter_pos = num_train_pos - num_avail_pos;
  ...
  */
  void abc_train_class(PartApp &part_app, int pidx, bool bBootstrap)
  {
    ...
    const AnnotationList &poslist = part_app.m_train_annolist;
    ...
    /** 
        currently we only support the new way of counting number of positive/negative examples
     */
    assert(part_app.m_abc_param.has_param_version());
    assert(part_app.m_abc_param.has_num_train_jitter() == false);
    assert(part_app.m_abc_param.has_num_train_bootstrap() == false);
    ...
    ExamplesCounting::init(part_app.m_abc_param, part_app.m_part_conf.part(pidx),
                           poslist, neglist, bootstrap_annolist);
    ...
    int part_image_offset = (int)ceil(sqrt(square((float)window_param.part(pidx).window_size_x()) + 
                                           square((float)window_param.part(pidx).window_size_y()))/2) + (int)(1.1*abc_param.desc_size()) + 1;
      ...
      for (int imgidx = 0; imgidx < (int)bootstrap_annolist.size(); ++imgidx) {
        ...
        for (int rectidx = 0; rectidx < (int)rects.size(); ++rectidx) {
          ...
          bool bres = compute_part_bbox_features_scale(abc_param, window_param, 
                                                       rects[rectidx], kmaimg, pidx, rects_scale[rectidx], 
    ...
    /* add required number of bootstrapping exampels */
    if (bBootstrap) {
    ...
    for (uint imgidx = 0; imgidx < poslist.size(); ++imgidx) {
      ...
      if (!bAddImageBorder) {
        ...
        input_image = add_image_border(_input_image, added_offset);
        cout << "added offset: " << added_offset << endl;
        delete _input_image;
      }
      ...
      for (uint ridx = 0; ridx < poslist[imgidx].size(); ++ridx) {
        ...
        PartBBox bbox;
        get_part_bbox(poslist[imgidx][ridx], part_conf.part(pidx), bbox);
        ...
        for (int jitidx = 0; jitidx < ExamplesCounting::num_jitter_per_pos && 
               ExamplesCounting::added_pos_jitter < ExamplesCounting::num_jitter_pos; ++jitidx) {
    ...
    for (uint imgidx = 0; imgidx < neglist.size(); ++imgidx) {
      ...
      while (ExamplesCounting::added_neg_curimg < ExamplesCounting::num_neg_random_per_image) {
        ...
        sample_random_partrect(gen_01, 
                               ...
                               min_pos_x, max_pos_x, 
                               min_pos_y, max_pos_y,
                               window_param.part(pidx).window_size_x(), 
                               window_param.part(pidx).window_size_y(), 
    ...
    part_app.saveClassifier(abc, pidx, bootstrap_type);
