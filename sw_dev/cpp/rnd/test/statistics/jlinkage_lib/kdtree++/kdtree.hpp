...
/*
      ...
      typedef _Region<_Val, typename _Acc::result_type, _Acc, _Cmp>
        _Region;
*/
      typedef _Node_compare<_Val, _Acc, _Cmp> __Node_compare;
...
*/
      ...
      size_t
        count_within_range(const_reference __V, subvalue_type const __R) const throw ()
        {
          ...
          __Region __region(_M_acc,__V,__R);
          return this->count_within_range(__region);
        }

      size_t
        count_within_range(__Region const& __REGION) const throw ()
        {
          if (!_M_get_root()) return 0;

          __Region __bounds(__REGION);
      ...
      template <typename SearchVal, class Visitor>
        ...
        visit_within_range(SearchVal V, subvalue_type const R, Visitor visitor) const throw ()
        {
          ...
          __Region region(_M_acc,V,R);
          return this->visit_within_range(region, visitor);
        }

      template <class Visitor>
        Visitor
        visit_within_range(__Region const& REGION, Visitor visitor) const throw ()
        {
          if (_M_get_root())
            {
              __Region bounds(REGION);
      ...
      template <typename SearchVal, typename _OutputIterator>
                          ...
                          _OutputIterator __outIT) const throw ()
		{
          ...
          __Region __region(mK,_M_acc,__V,__R);
      ...
      template <typename _OutputIterator>
        _OutputIterator
        find_within_range(__Region const& __REGION,
                          _OutputIterator __outIT) const throw ()
        {
          if (_M_get_root())
            {
      ...
      template <typename SearchVal>
        ...
        find_nearest(SearchVal __V, subvalue_type const __Max_R ) const throw ()
        {
          ...
          __Region __region(_M_acc,__V);  // note: zero-area!
          typename __Region::_CenterPt __pt(__region,__Max_R);
      ...
      template <typename SearchVal, class Predicate>
        ...
        find_nearest_if(SearchVal __V, subvalue_type const __Max_R, Predicate predicate ) const throw ()
        {
          if (!_M_get_root())
             return std::pair<const_iterator,distance_type>(this->end(),__Max_R);
          __Region __region(mK,_M_acc,__V);  // note: zero-area!
          typename __Region::_CenterPt __pt(__region,__Max_R);
          return this->find_nearest_if(__pt,predicate);
        }

      std::pair<const_iterator,distance_type>
        find_nearest( typename __Region::_CenterPt const& __CENTER ) const throw ()
        {
          if (_M_get_root())
          {
             // note: we set the initial 'bounds' to the exact point.
             // they expand from there outwards.
             __Region __bounds(__CENTER.first);
      ...
      std::pair<const_iterator,distance_type>
        find_nearest_if( typename __Region::_CenterPt const& __CENTER, Predicate predicate ) const throw ()
        {
          if (_M_get_root())
          {
             // note: we set the initial 'bounds' to the exact point.
    ...
    protected:
      ...
      void _M_check_children( _Link_const_type child, _Link_const_type parent, size_t const level, bool to_the_left )
      {
         ...
         if (child)
         {
            __Node_compare compare(level % mK,_M_acc);
      ...
      {
        if (__Node_compare(__L % mK,_M_acc)(__V, __N))
          {
            if (!_S_left(__N))
      ...
      _M_get_j_min( std::pair<_Link_type,size_t> const node, size_t const level) throw ()
      {
        ...
        __Node_compare compare(node.second % mK,_M_acc);
      ...
      {
        __Node_compare compare(__L % mK,_M_acc);
      ...
      size_t
        _M_count_within_range(_Link_const_type __N, __Region const& __REGION,
                             __Region const& __BOUNDS,
                             size_t const __L) const throw ()
        {
           size_t count = 0;
      ...
      template <class Visitor>
        ...
        _M_visit_within_range(Visitor visitor,
                             _Link_const_type N, __Region const& REGION,
                             __Region const& BOUNDS,
      ...
      template <typename _OutputIterator>
        ...
        _M_find_within_range(_OutputIterator __outIT,
                             _Link_const_type __N, __Region const& __REGION,
                             __Region const& __BOUNDS,
                             size_t const __L) const throw ()
        {
          if (__REGION.encloses(_S_value(__N)))
       ...
       // And so it goes ...
         ...
         template <class Predicate>
         std::pair<const_iterator,distance_type>
        _M_find_nearest( _Link_const_type __N, typename __Region::_CenterPt __CENTER,
                             __Region const& __BOUNDS,
                             size_t const __L,
