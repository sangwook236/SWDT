...
RandomSampler::RandomSampler(
	...
	float (*nPtPairDistance)(const std::vector<float> &nVecI, const std::vector<float> &nVecJ),
	...
	mPtPairDistance = nPtPairDistance;
...
void RandomSampler::AddPointUpdateProbability(unsigned int nIndex){
	...
	if(mNFSType == NFST_EXP){
		...
		for(unsigned int j=0; j < mDataPoints.size(); j++){
			...
			eucDist = /*VecEuclideanDist*/mPtPairDistance(*mDataPoints[nIndex]->mCoord, *mDataPoints[j]->mCoord);
	...
	for(unsigned int i=0; i<mMaxAllocablePoints; i++){
		...
		for(unsigned int j=i+1; j < mMaxAllocablePoints; j++){
			if(!mDataPoints[j]->mActive)
				continue;
			// Compute euclidean distance
			eucDist = mPtPairDistance(*mDataPoints[i]->mCoord, *mDataPoints[j]->mCoord);
