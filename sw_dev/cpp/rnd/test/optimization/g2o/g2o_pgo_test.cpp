...
void pgo_test()
{
	...
	std::map<size_t, point_set_type> point_sets;
	{
		...
		//for (size_t fidx = 0; fidx < input_filepaths.size() && fidx < 20; fidx += 5)
		{
			...
			const camera_pose_type Tcaminit(Eigen::Translation3f(cx, cy, nearest_pt.z * 2.0f) * Eigen::AngleAxisf(M_PIf, Eigen::Vector3f(1.0f, 0.0f, 0.0f)));
	...
	// Refer to slam2d_tutorial(), ba_example(), slam3d_se3_pointxyz_test(), & slam3d_se3_test().
	{
		...
		// Construct a pose graph.
		{
			...
			// Add the odometry constraints (edges).
			{
				...
				//const auto information_matrix(Eigen::DiagonalMatrix<double, 6>(20.0, 20.0, 20.0, 20.0, 20.0, 20.0));
			...
			// Add the landmark constraints (edges).
			{
				...
				//const auto information_matrix(Eigen::DiagonalMatrix<double, 3>(40.0, 40.0, 40.0));
				for (const auto &nid: pose_graph)
				{
					const auto &cloud = std::get<0>(point_sets[nid]);  // Raw point cloud.
					//const auto &cloud = std::get<1>(point_sets[nid]);  // Downsampled point cloud.
					...
					for (size_t pidx = 0; pidx < cloud->size(); ++pidx)
					{
						...
						observation->setMeasurement(g2o::Vector3(pt.x, pt.y, pt.z));  // Relative measurement.
		...
		// Show PGO results.
		{
			...
			std::sample(pg_landmarks.begin(), pg_landmarks.end(), std::inserter(pg_landmarks_sampled, pg_landmarks_sampled.end()), num_samples, std::mt19937 { std::random_device{}() });
			for (const auto &l: pg_landmarks_sampled)
			{
				...
				std::cout << vtx_est.transpose() << std::endl;
			}
		}
