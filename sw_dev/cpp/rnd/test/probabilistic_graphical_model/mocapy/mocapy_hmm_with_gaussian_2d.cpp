...
// [ref] ${MOCAPY_HOME}/examples/hmm_gauss.cpp
void hmm_with_gaussian_2d()
{
	...
	means.set_shape(2, 2);
	means.set_values(mocapy::vec(0.0, 0.0, 10.0, 10.0));
	...
	mocapy::Node *to0 = mocapy::NodeFactory::new_gaussian_node(O_SIZE, "to0", false, useShrinkage, means);
	...
	mocapy::Node *mo0 = mocapy::NodeFactory::new_gaussian_node(O_SIZE, "mo0", true, useShrinkage);
	...
	while (it_no_improvement < 10)
	{
		...
		if (ll > bestLL)
		{
			...
			mdbn.save("./data/probabilistic_graphical_model/mocapy/hmm_with_gaussian_2d.dbn");
			bestLL = ll;
			it_no_improvement = 0;
		}
		else
	...
	//---------------------------------------------------------------
	mdbn.load("./data/probabilistic_graphical_model/mocapy/hmm_with_gaussian_2d.dbn");
