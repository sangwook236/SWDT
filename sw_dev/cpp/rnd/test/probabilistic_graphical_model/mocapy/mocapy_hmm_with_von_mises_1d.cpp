...
// [ref] ${MOCAPY_HOME}/examples/hmm_vonmises.cpp
void hmm_with_von_mises_1d()
{
	...
	const double mus_arr[] = { 0.0, 2.0 };
	std::vector<double> mus(mus_arr, mus_arr + H_SIZE);
	const double kappas_arr[] = { 80.0, 80.0 };
	std::vector<double> kappas(kappas_arr, kappas_arr + H_SIZE);
	...
	mocapy::Node *to0 = mocapy::NodeFactory::new_vonmises_node("to0", mus, kappas);
	...
	std::cout << "von Mises parameters: " << to0->get_parameters() << std::endl;
	...
	mocapy::Node *mo0 = mocapy::NodeFactory::new_vonmises_node("mo0");
	...
	while (it_no_improvement < 10)
	{
		...
		if (ll > bestLL)
		{
			...
			mdbn.save("./data/probabilistic_graphical_model/mocapy/hmm_with_von_mises_1d.dbn");
			bestLL = ll;
			it_no_improvement = 0;
		}
		else
	...
	//---------------------------------------------------------------
	mdbn.load("./data/probabilistic_graphical_model/mocapy/hmm_with_von_mises_1d.dbn");
