...
// [ref] ${MOCAPY_HOME}/examples/hmm_torus.cpp
void hmm_with_von_mises_2d()
{
	...
	mocapy::MDArray<double> kappas;
	kappas.set_shape(H_SIZE, 3);
	kappas.set(0, 0, 100);
	kappas.set(0, 1, 100);
	kappas.set(0, 2, 50);
	...
	mocapy::MDArray<double> mus;
	mus.set_shape(H_SIZE, 2);
	mus.set(0, 0, 0.5);
	mus.set(0, 1, 0.5);
	...
	mocapy::Node *to0 = mocapy::NodeFactory::new_vonmises2d_node("to0", mus, kappas);
	...
	mocapy::Node *mo0 = mocapy::NodeFactory::new_vonmises2d_node("mo0");
	...
	while (it_no_improvement < 10)
	{
		...
		if (ll > bestLL)
		{
			...
			mdbn.save("./data/probabilistic_graphical_model/mocapy/hmm_with_von_mises_2d.dbn");
			bestLL = ll;
			it_no_improvement = 0;
		}
		else
	...
	//---------------------------------------------------------------
	mdbn.load("./data/probabilistic_graphical_model/mocapy/hmm_with_von_mises_2d.dbn");
