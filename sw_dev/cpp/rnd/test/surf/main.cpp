...
void loadIpoints(std::string fn, std::vector< Ipoint >& keys, bool bVerbose = false);
void saveIpoints(std::string fn, const std::vector< Ipoint >& keys, bool bVerbose = false, bool bLaplacian = true);
...
int main (int argc, char **argv)
{
	...
	std::string sRegionFile = "";

	// Print command line help
	if (1 == argc)
	{
		std::cerr << "./surf -i img.pgm -o img.surf [options]\n"
	...
	FastHessian fh(
		&iimage, /* pointer to integral image */
		ipts,
	...
	if (bVerbose)
		std::cout << "Detection time: " << osmapping::os_TimeDiff(&tim2, &tim1) << " ms" << std::endl;
...
void loadIpoints(std::string sFileName, std::vector<Ipoint>& ipts, bool bVerbose) 
{
	...
	// create a new interest point std::vector
	ipts.clear();
	ipts.resize(count);
