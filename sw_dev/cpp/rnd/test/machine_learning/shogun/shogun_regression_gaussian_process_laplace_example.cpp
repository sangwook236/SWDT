...
shogun::CModelSelectionParameters * build_tree(shogun::CInferenceMethod *inf, shogun::CLikelihoodModel *lik, shogun::CKernel *kernel)
{
	...
	c1->append_child(c2);
	c2->build_values(0.5, 4.0, shogun::R_LINEAR);
	...
	shogun::CModelSelectionParameters *c43 = new shogun::CModelSelectionParameters("df");
	c3->append_child(c43);
	c43->build_values(500.0, 1000.0, shogun::R_LINEAR);
...
// [ref] ${SHOGUN_HOME}/examples/undocumented/libshogun/regression_gaussian_process_laplace.cpp
void regression_gaussian_process_laplace_example()
{
	...
	shogun::CStudentsTLikelihood *lik = new shogun::CStudentsTLikelihood();

	// Allocate our inference method
	shogun::CLaplacianInferenceMethod *inf = new shogun::CLaplacianInferenceMethod(test_kernel, features, mean, labels, lik);
	SG_REF(inf);
	...
	shogun::CGradientModelSelection *grad_search = new CGradientModelSelection(root, grad);
