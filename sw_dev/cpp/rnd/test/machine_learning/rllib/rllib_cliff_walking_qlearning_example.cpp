...
void make_experiment(CRITIC& critic, const Q& q)
{
	...
	for (episode = 0, frame = 0; episode < NB_EPISODES; ++episode)
	{
		...
		if (episode % FRAME_PERIOD == 0)
		{
			...
			Cliff::draw_visited(
				"data/rllib-cliff-qlearning", frame++,
...
// Q-learning.
// REF [file] >> ${RLlib_HOME}/examples/example-001-002-cliff-walking-qlearning.cc.
void cliff_walking_qlearning_example()
{
	...
	auto action_begin = rl::enumerator<local::Action>(rl::problem::cliff_walking::actionNorth);
	auto action_end = action_begin + rl::problem::cliff_walking::actionSize;

	auto critic = rl::gsl::q_learning<local::State, local::Action>(
		theta,
