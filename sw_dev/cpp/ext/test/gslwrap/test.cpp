...
void Histogram()
{
	...
	gsl::random_generator generator; // default constructor takes seed from ENV variable
  	int nSamples = 10000;
...
void VectorFloat()
{
	gsl::vector_float test;
	gsl::vector_float mal(10);
	for (int i = 0; i < 10; ++i)
		mal[i] = (float)i;
...
void VectorView()
{
	...
	int size = 5;
	gsl::matrix_float m(size, size);
	gsl::matrix_float ver(size, size);  ver.set_all(5.0);
	gsl::vector_float v(size);
	...
	gsl::vector_float_view col_viewer = m.column(3);
	gsl::vector_float_view viewer = m.row(3);
