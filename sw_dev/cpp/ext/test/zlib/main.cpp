...
#  define TESTFILE "zlib_data\\foo-gz"
#else
#  define TESTFILE "zlib_data\\foo.gz"
#endif
...
{
	//
	std::ifstream stream("zlib_data\\healthcare_questionnaire.xml", std::ios::in);
	if (!stream.is_open())
	{
	...
	// compression
	{
		const std::string fname("zlib_data\\healthcare_questionnaire.format");
