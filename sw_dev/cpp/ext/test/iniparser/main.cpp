...
int main(int argc, char * argv[])
{
	...
	if (argc<2) {
		...
		status = parse_ini_file("iniparser_data\\example.ini");
	} else {
...
void create_example_ini_file(void)
{
	...
	ini = fopen("iniparser_data\\example.ini", "w");
	fprintf(ini, "\n\
#\n\
...
int parse_ini_file(char * ini_name)
{
	...
	//
	{
		FILE *fp = fopen("iniparser_data\\result1.ini", "w");
	...
	//
	{
		FILE *fp = fopen("iniparser_data\\result2.ini", "w");
		iniparser_dump(ini, fp);
		fclose(fp);
	}
	...
	//
	{
		FILE *fp = fopen("iniparser_data\\result3.ini", "w");
