...
typedef dlib::matrix<double, 0, 1> column_vector;       // Must be a dlib::matrix type.
typedef dlib::matrix<double, 0, 1> sample_type;         // Can be anything you want.
typedef dlib::matrix<double, 0, 1> feature_vector_type; // Must be dlib::matrix or some kind of sparse vector.
...
!*/
{
    ...
    w0 = dlib::rowm(weights, dlib::range(0,2));
    w1 = dlib::rowm(weights, dlib::range(3,5));
    w2 = dlib::rowm(weights, dlib::range(6,8));

    column_vector scores(3);
    scores = dlib::dot(w0, sample), dlib::dot(w1, sample), dlib::dot(w2, sample);

    return dlib::index_of_max(scores);
}
...
class three_class_classifier_problem : public dlib::structural_svm_problem_threaded<column_vector, feature_vector_type>
{
    /*!
        ...
        as possible:
            F(x_i,y_i) > max{over all incorrect labels_ of x_i} F(x_i, y_incorrect)
        ...
        problem precisely, we first introduce some notation:
            ...
            - The number of data samples_ is N.
...
public:
    ...
    // provide the things discussed above.  This is the number of training samples_, the
    ...
    // is take in the training samples_ and their labels_ as well as a number indicating how
    ...
    three_class_classifier_problem (const std::vector<sample_type>& samples, const std::vector<int>& labels, const unsigned long num_threads)
	: dlib::structural_svm_problem_threaded<column_vector, feature_vector_type>(num_threads),
      samples_(samples), labels_(labels)
    {}

    feature_vector_type make_psi(const sample_type& x, const int label) const
    /*!
        ensures
    ...
    !*/
    {
        ...
        if (label == 0)
			dlib::set_rowm(psi, dlib::range(0,2)) = x;
        else if (label == 1)
			dlib::set_rowm(psi, dlib::range(3,5)) = x;
        else  // the label must be 2 
			dlib::set_rowm(psi, dlib::range(6,8)) = x;
    ...
    // the number of training samples_.  We do this by defining the following virtual
    // functions.
    virtual long get_num_dimensions() const { return samples_[0].size() * 3; }
    virtual long get_num_samples() const { return samples_.size(); }
    ...
    //               - #loss == LOSS(idx,Y) 
    //               - #psi == PSI(X,Y) 
    virtual void separation_oracle(const long idx, const column_vector& current_solution, scalar_type& loss, feature_vector_type& psi) const 
    {
        ...
        scores = dlib::dot(dlib::rowm(current_solution, dlib::range(0,2)), samples_[idx]),
			     dlib::dot(dlib::rowm(current_solution, dlib::range(3,5)), samples_[idx]),
			     dlib::dot(dlib::rowm(current_solution, dlib::range(6,8)), samples_[idx]);
        ...
        const int max_scoring_label = dlib::index_of_max(scores);
        ...
        if (max_scoring_label == labels_[idx])
            loss = 0;
        else
            loss = 1;
        ...
        psi = make_psi(samples_[idx], max_scoring_label);
    }
...
// REF [file] >> ${DLIB_HOME}/examples/svm_struct_ex.cpp
void svm_struct_example()
{
	...
	const local::column_vector& weights = local::train_three_class_classifier(samples, labels);
	...
	for (unsigned long i = 0; i < samples.size(); ++i)
		std::cout << "predicted label for sample[" << i << "]: " << local::predict_label(weights, samples[i]) << std::endl;
}
